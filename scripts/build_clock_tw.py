#!/usr/bin/env python3
"""
Build the Taiwan property clock (Structure x Flow) from raw quarterly series and inject
it into my/clock.html (Taiwan market block). Same five-force engine as the US pilot; only
the inputs differ, so the compute half is identical to build_clock_us.py.

INPUT: a JSON file of raw quarterly series (scripts/.twdata/tw_raw.json), shape:
{
...
Usage:
    python3 scripts/build_clock_tw.py                 # read .twdata/tw_raw.json, write + inject
    python3 scripts/build_clock_tw.py --no-inject     # write data/clock-tw.json only
...
INJSON  = os.path.join(os.path.dirname(os.path.abspath(__file__)), '.twdata', 'tw_raw.json')
OUTJSON = os.path.join(ROOT, 'data', 'clock-tw.json')
PAGE    = os.path.join(ROOT, 'my', 'clock.html')
MARK_A, MARK_B = '/*CLOCK_TW_START*/', '/*CLOCK_TW_END*/'
    ...
    if '--no-inject' not in sys.argv:
        ...
        if MARK_A in h and MARK_B in h:
            ...
            h = re.sub(re.escape(MARK_A)+'.*?'+re.escape(MARK_B),
                       MARK_A+'var D_TW = '+compact+';'+MARK_B, h, count=1, flags=re.S)
