#!/usr/bin/env python3
"""
Build the Malaysia property clock (Structure x Flow) from raw quarterly series and inject
it into my/clock.html (Malaysia market block). Same five-force engine as the US pilot; only
...
INJSON  = os.path.join(os.path.dirname(os.path.abspath(__file__)), '.mydata', 'my_raw.json')
OUTJSON = os.path.join(ROOT, 'data', 'clock-my.json')
PAGE    = os.path.join(ROOT, 'my', 'clock.html')
MARK_A, MARK_B = '/*CLOCK_MY_START*/', '/*CLOCK_MY_END*/'
    ...
    if '--no-inject' not in sys.argv:
        ...
        if MARK_A in h and MARK_B in h:
            ...
            h = re.sub(re.escape(MARK_A)+'.*?'+re.escape(MARK_B),
                       MARK_A+'var D_MY = '+compact+';'+MARK_B, h, count=1, flags=re.S)
