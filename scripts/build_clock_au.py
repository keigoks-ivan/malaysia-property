#!/usr/bin/env python3
"""
Build the Australia property clock (Structure x Flow) from raw quarterly series.
Same five-force engine as the US pilot (and the JP/TW/MY builders); only the
inputs differ, so the compute half is identical to build_clock_jp.py. Unlike
JP/TW/MY, Australia has no legacy market page to inject into — this script only
ever writes data/clock-au.json; the page-injection step is guarded to skip
cleanly (never opens/creates any HTML) since no such page exists yet.

INPUT: a JSON file of raw quarterly series (scripts/.audata/au_raw.json), shape:
{
  "q":      ["1975Q1", ...],        # quarter labels, contiguous
  "price":  [ ... ],                # BIS Residential Property Prices, Australia (nominal index) -> judged + valuation numerator
  "pti":    [ ... ],                # not sourced for AU (no clean official PTI) -> valuation falls back to price/income [optional]
  "months": [ ... ],                # ABS Building Approvals (8731.0), dwelling units, ORIGINAL -> supply (low = tight)  [optional]
  "vacancy":[ ... ],                # SQM Research national residential rental vacancy rate (%) -> supply (low = tight) [optional]
  "gap":    [ ... ],                # not sourced for AU (no household formation - completions series) [optional]
  "pop":    [ ... ],                # ABS Estimated Resident Population (level) -> population (growth = +)   [optional]
  "credit": [ ... ],                # RBA Table D2 housing credit (owner-occ + investor, level) -> money (real growth = +) [optional]
  "rate":   [ ... ],                # RBA cash rate % (spliced with interbank overnight cash rate pre-1990) -> money (real, easing = +)  [optional]
  "cpi":    [ ... ],                # ABS CPI (6401.0) index -> deflator for credit & rate [optional]
  "gdp":    [ ... ],                # ABS National Accounts (5206.0) nominal GDP (level) -> economy (YoY = +)         [optional]
  "unemp":  [ ... ],                # ABS Labour Force (6202.0) unemployment rate % (level) -> economy (falling = +) [optional]
  "income": [ ... ]                 # ABS National Accounts (5206.0) household gross disposable income (level) -> economy (YoY = +); valuation denominator (no pti for AU)
}
...
Axis 1975Q1-2026Q1; credit starts 1990Q1 (RBA owner-occ/investor split begins
1990-01), vacancy starts 2005Q1 (SQM series begins Jan 2005), months (building
approvals) starts 1983Q1 -- these nulls are correct provenance gaps, not missing
data to backfill.

This market shares MY/TW/JP's 6y (24q) trailing standardisation window (not the
US's 10y default); override with --win N.

Usage:
    python3 scripts/build_clock_au.py                 # read .audata/au_raw.json, write data/clock-au.json
    python3 scripts/build_clock_au.py --no-inject     # write data/clock-au.json only (same as default; AU has no page)
    python3 scripts/build_clock_au.py --selftest      # run on synthetic data, no files needed
    python3 scripts/build_clock_au.py --win 40        # use a 10y window (if history allows)
"""
import json, math, os, sys
import statistics as st

ROOT    = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
INJSON  = os.path.join(os.path.dirname(os.path.abspath(__file__)), '.audata', 'au_raw.json')
OUTJSON = os.path.join(ROOT, 'data', 'clock-au.json')
PAGE    = os.path.join(ROOT, 'my', 'clock.html')
MARK_A, MARK_B = '/*CLOCK_AU_START*/', '/*CLOCK_AU_END*/'
...
def main():
    ...
    if not os.path.exists(INJSON):
        print(f'no input at {INJSON} — run --selftest, or place raw AU series there first'); sys.exit(1)
    ...
    # Australia has no legacy market page (unlike JP/TW/MY) — never open/create one.
    # --no-inject is the default-equivalent path here; injection only proceeds if the
    # page exists AND already carries the AU markers, otherwise it skips cleanly.
    if '--no-inject' not in sys.argv:
        if not os.path.exists(PAGE):
            print(f'no page at {PAGE}; AU has no market page to inject into — wrote JSON only')
        else:
            h = open(PAGE, encoding='utf-8').read()
            if MARK_A in h and MARK_B in h:
                import re
                h = re.sub(re.escape(MARK_A)+'.*?'+re.escape(MARK_B),
                           MARK_A+'var D_AU = '+compact+';'+MARK_B, h, count=1, flags=re.S)
                open(PAGE, 'w', encoding='utf-8').write(h)
                print(f'injected into {PAGE}')
            else:
                print('AU markers not yet in page; wrote JSON only (add toggle first)')
